...
trait Decoder[F[_], A, B] { outer =>
  ...
  def map[C](f: B => C)(implicit F: Functor[F]): Decoder[F, A, C] = new Decoder[F, A, C] {
    override def decode(a: A): F[C] = outer.decode(a).map(f)
  }
}
...
object EncoderK {
  def apply[C[_], F[_], Out](implicit ev: EncoderK[C, F, Out]): EncoderK[C, F, Out] = ev

  implicit def fromProjectOp[C[_], F[_], Repr, Out](
    implicit gen: Generic.Aux[C[_], Repr],
    handleEncode: HandleEncode[C, F, Repr, Out]
  ): EncoderK[C, F, Out] = new EncoderK[C, F, Out] {
    override def apply(cf: ~>[C, F]): ~>[C, ConstT[F, Out, ?]] = new (C ~> ConstT[F, Out, ?]) {
...
trait HandleEncode[C[_], F[_], T, Out] { outer =>
  ...
  def contramap[T1](f: T1 => T): HandleEncode[C, F, T1, Out] = new HandleEncode[C, F, T1, Out] {
    override def apply(cf: ~>[C, F]): (T1) => F[Out] = (x: T1) => outer(cf)(f(x))
  }
...
object HandleEncode {
  ...
  implicit def handlerCnil[C[_], F[_], Out]: HandleEncode[C, F, CNil, Out] =
    new HandleEncode[C, F, CNil, Out] {
      override def apply(cf: ~>[C, F]): (CNil) => F[Out] = _.impossible
    }
  ...
  ): HandleEncode[C, F, CA :+: T, Out] = new HandleEncode[C, F, CA :+: T, Out] {
    override def apply(cf: ~>[C, F]): (:+:[CA, T]) => F[Out] =
...
object Server {
  def apply[Request, Response, Decoded[_]: Functor]: Server[Request, Response, Decoded] =
